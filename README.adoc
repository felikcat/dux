...
.*Dux* is simplistic installer for **pure** Arch Linux; no different release schedules and breakage that Manjaro inflicts, and no third-party repos.
- It is also simple to modify for accomodating your own preferences, as it's programmed in Bash.
...
- A focus on maintaining a stable OS, such as using Btrfs snapshots automatically.
...
== 1. [Optional / Security] Verify ISO authenticity
...
.*Windows*
. Install https://gpg4win.org/thanks-for-download.html[Gpg4Win].
. Open `gpg.exe`; it should say "Go ahead and type your message".
. Follow the Linux instructions below.
